...
namespace FishNet.Managing.Timing
{
    ...
    public sealed class TimeManager : MonoBehaviour
    {
        ...
        internal void TickUpdate()
        {
            ...
            var beforeTick = _updateOrder == UpdateOrder.BeforeTick;
            if (beforeTick)
            {
                OnUpdate?.Invoke();
                MethodLogic();
        ...
        private void IncreaseTick()
        {
            ...
            var tickDelta = TickDelta;
            var timePerSimulation = isServer ? tickDelta : _adjustedTickDelta;
            ...
            var ticksCount = Mathf.FloorToInt((float) (_elapsedTickTime / timePerSimulation));
            if (ticksCount > 1)
            ...
            if (_allowTickDropping && !_networkManager.IsServer)
                ...
                if (ticksCount > _maximumFrameTicks)
                    _elapsedTickTime = timePerSimulation * _maximumFrameTicks;

            var variableTiming = _timingType == TimingType.Variable;
            var frameTicked = FrameTicked;
        ...
        /// <summary>
        ///     When using TimeManager for physics timing, this is called immediately before physics simulation will occur for the
        ///     tick.
        ...
        /// <summary>
        ///     When using TimeManager for physics timing, this is called immediately after the physics simulation has occured for
        ///     the tick.
        ...
        /// </summary>
        [Tooltip("When to invoke OnUpdate and other Unity callbacks relayed by the TimeManager.")] [SerializeField]
        private UpdateOrder _updateOrder = UpdateOrder.BeforeTick;

        /// <summary>
        ///     Timing for sending and receiving data.
        /// </summary>
        [Tooltip("Timing for sending and receiving data.")] [SerializeField]
        ...
        ///     This could result in a temporary desynchronization but will prevent the client falling further behind on ticks by
        ///     repeatedly running the logic cycle multiple times per frame.
        /// </summary>
        [Tooltip(
        ...
        /// <summary>
        ///     How often in seconds to a connections ping. This is also responsible for approximating server tick. This value does
        ///     not affect prediction.
        /// </summary>
        internal byte PingInterval => _pingInterval;

        /// <summary>
        ///     How often in seconds to update prediction timing. Lower values will result in marginally more accurate timings at
        ...
        /// </summary>
        private readonly SystemStopwatch _pingStopwatch = new();
        ...
        /// </summary>
        private readonly MovingAverage _pingAverage = new(5);
        ...
        /// <summary>
        ///     When steps to be sent to clients are equal to or higher than this value in either direction a reset steps will be
        ///     sent.
        /// </summary>
        ...
        private void TrySendPing(uint? tickOverride = null)
        {
            ...
            var multiplier = _receivedPong ? 1f : 1.5f;

            requiredTime = (long) (requiredTime * multiplier);
            var requiredTicks = TimeToTicks(pingInterval * multiplier);
        ...
        public double GetTickPercent()
        {
            ...
            var percent = _elapsedTickTime / delta * 100d;
            return percent;
        }
        ...
        public PreciseTick GetPreciseTick(TickType tickType)
        {
            ...
            if (tickType == TickType.Tick) return GetPreciseTick(Tick);

            if (tickType == TickType.LocalTick) return GetPreciseTick(LocalTick);

            if (tickType == TickType.LastPacketTick)
            {
        ...
        {
            if (tickType == TickType.LocalTick) return TicksToTime(LocalTick);

            if (tickType == TickType.Tick) return TicksToTime(Tick);
        ...
        {
            var currentPt = GetPreciseTick(TickType.Tick);

            long tickDifference = currentPt.Tick - preciseTick.Tick;
            var percentDifference = currentPt.Percent - preciseTick.Percent;

            /* If tickDifference is less than 0 or tickDifference and percentDifference are 0 or less
             * then the result would be negative. */
            var negativeValue = tickDifference < 0 || (tickDifference <= 0 && percentDifference <= 0);

            if (!allowNegative && negativeValue)
                return 0d;

            var tickTime = TimePassed(preciseTick.Tick, true);
            var percent = percentDifference / 100;
            var percentTime = percent * TickDelta;
        ...
        public double TimePassed(uint previousTick, bool allowNegative = false)
        {
            ...
            if (currentTick >= previousTick) return TicksToTime(currentTick - previousTick);
            //Difference would be negative.
            if (!allowNegative) return 0d;

            var difference = TicksToTime(previousTick - currentTick);
        ...
        public uint TimeToTicks(double time, TickRounding rounding = TickRounding.RoundNearest)
        {
            ...
            if (rounding == TickRounding.RoundDown)
        ...
        public uint TickToLocalTick(uint tick)
        {
            ...
            var result = LocalTick - difference;
            if (result <= 0)
                result = 0;
        ...
        {
            var requiredTicks = TimeToTicks(_timingInterval);
            var tick = Tick;
        ...
        internal void ParseTimingUpdate()
        {
            ...
            var expected = (uint) (TickRate * _timingInterval);
            long difference;
            //If ticking too fast.
            if (_clientTicks > expected)
                difference = _clientTicks - expected;
            //Not ticking fast enough.
            else
                difference = (expected - _clientTicks) * -1;
