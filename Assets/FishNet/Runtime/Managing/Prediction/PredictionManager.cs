...
namespace FishNet.Managing.Predicting
{
    ...
    public sealed class PredictionManager : MonoBehaviour
    {
        ...
        {
            var removed = _rigidbodies.Remove(c);
            ...
            if (!removed)
            {
                ...
                _rigidbodies.Clear();
                foreach (var item in _componentCache)
        ...
        [APIExclude]
        [CodegenMakePublic] //To internal.
        public void InvokeOnReplicateReplay_Internal(UnityScene scene, uint tick, PhysicsScene ps, PhysicsScene2D ps2d,
            bool before)
        {
            _isReplaying = before;
            if (before)
        ...
        /// <returns></returns>
        public bool IsReplaying(UnityScene scene)
        {
        ...
        /// <summary>
        ///     True to drop replicates from clients which are being received excessively. This can help with attacks but may cause
        ///     client to temporarily desynchronize during connectivity issues.
        ///     When false the server will hold at most up to 3 seconds worth of replicates, consuming multiple per tick to clear
        ///     out the buffer quicker. This is good to ensure all inputs are executed but potentially could allow speed hacking.
        /// </summary>
        ...
        /// <summary>
        ///     Maximum number of replicates a server can queue per object. Higher values will put more load on the server and add
        ///     replicate latency for the client.
        /// </summary>
        ...
        public void SetMaximumServerReplicates(ushort value)
        {
            _maximumServerReplicates =
                (ushort) Mathf.Clamp(value, MINIMUM_REPLICATE_QUEUE_SIZE, MAXIMUM_REPLICATE_QUEUE_SIZE);
        }
        ...
        /// <summary>
        ///     Maximum number of excessive replicates which can be consumed per tick. Consumption count will scale up to this
        ///     value automatically.
        /// </summary>
        internal byte GetMaximumConsumeCount()
        {
            return _maximumConsumeCount;
        }
...
//WebGL uses reliable so no reason to use redundancy.
...
#endif
        /// <summary>
        ///     True to allow clients to use predicted spawning. While true, each NetworkObject prefab you wish to predicted spawn
        ///     must be marked as to allow this feature.
        /// </summary>
        ...
        /// <summary>
        ///     Maximum number of Ids to reserve on clients for predicted spawning. Higher values will allow clients to send more
        ///     predicted spawns per second but may reduce availability of ObjectIds with high player counts.
        /// </summary>
        /// <returns></returns>
        ...
        /// </summary>
        [NonSerialized] private readonly HashSet<UnityEngine.Component> _rigidbodies = new();

        /// <summary>
        ///     Cache to remove null entries from _rigidbodies.
        /// </summary>
        [NonSerialized] private readonly HashSet<UnityEngine.Component> _componentCache = new();
        ...
        /// </summary>
        private readonly HashSet<UnityScene> _replayingScenes = new(new SceneHandleEqualityComparer());
