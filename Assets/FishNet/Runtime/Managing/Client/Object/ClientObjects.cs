...
namespace FishNet.Managing.Client
{
    ...
    public partial class ClientObjects : ManagedObjects
    {
        ...
        /// </summary>
        private readonly ClientObjectCache _objectCache;
        ...
        internal void PredictedSpawn(NetworkObject networkObject, NetworkConnection ownerConnection)
        {
            ...
            if (predictedObjectIds.Count == 0)
            {
                NetworkManager.LogError(
                    $"Predicted spawn for object {networkObject.name} failed because no more predicted ObjectIds remain. This usually occurs when the client is spawning excessively before the server can respond. Increasing ReservedObjectIds within the ServerManager component or reducing spawn rate could prevent this problem.");
                return;
            }

            networkObject.PreinitializePredictedObject_Client(NetworkManager, predictedObjectIds.Dequeue(),
                ownerConnection, NetworkManager.ClientManager.Connection);
        ...
        public void WriteSpawn(NetworkObject nob, Writer writer)
        {
            ...
            WriteChangedTransformProperties(nob, sceneObject, false, headerWriter);
        ...
        private void RegisterAndDespawnSceneObjects(Scene s)
        {
            ...
            for (var i = 0; i < nobs.Written; i++)
            {
                ...
                UpdateNetworkBehavioursForSceneObject(nob, false);
        ...
        {
            var usedObjectId = reader.ReadNetworkObjectId();
        ...
        internal void CacheSpawn(PooledReader reader)
        {
             ...
             * to predicted spawn. */
            if (!NetworkManager.IsServerOnly && Spawned.TryGetValue(objectId, out var nob))
            {
                //If not predicted the nob should not be in spawned.
                if (!nob.PredictedSpawner.IsValid)
                {
                    NetworkManager.LogError(
                        $"Received a spawn objectId of {objectId} which was already found in spawned, and was not predicted.");
                }
                //Everything is proper, apply RPC links.
                else
                {
                    var linkReader = ReaderPool.GetReader(rpcLinks, NetworkManager);
            ...
            _objectCache.AddSpawn(NetworkManager, collectionId, objectId, initializeOrder, ownerId, st, componentIndex,
                rootObjectId, parentObjectId, parentComponentIndex, prefabId, localPosition, localRotation, localScale,
                sceneId, rpcLinks, syncValues);
        }
        ...
        ///     Parses the packets within the cache and ensures objects are spawned and despawned before their sync values are
        ///     applied.
        ///     This ensures there is no chance a sync value is referencing a spawned object which does not exist yet due to it
        ///     normally being spawned later in the cache.
        /// </summary>
        ...
        internal NetworkObject GetNestedNetworkObject(CachedNetworkObject cnob)
        {
            ...
            {
                NetworkManager.LogError(
                    $"Nested spawned object with componentIndex of {componentIndex} and a parentId of {rootObjectId} could not be spawned because parent was not found.");
                return null;
            }
            ...
            {
                NetworkManager.LogError(
                    $"Nested spawned object with componentIndex of {componentIndex} could not be found as a child NetworkObject of {rootNob.name}.");
                return null;
            }
        ...
        internal NetworkObject GetInstantiatedNetworkObject(CachedNetworkObject cnob)
        {
            ...
            {
                networkManager.LogError(
                    $"PrefabObjects collection is not found for CollectionId {collectionId}. Be sure to add your addressables NetworkObject prefabs to the collection on server and client before attempting to spawn them over the network.");
                return null;
            }
            ...
            if (!networkManager.IsHost)
            {
                ...
                if (hasParent)
                {
                    ...
                    if (nob == null)
                    {
                        ...
                        networkManager.LogError(
                            $"NetworkObject not found for ObjectId {objectId}. Prefab {prefab.name} will be instantiated without parent synchronization.");
                    }
                    else
                    {
                        ...
                        if (cnob.ParentIsNetworkBehaviour)
                        {
                            ...
                            else
                            {
                                var prefab = prefabObjects.GetObject(false, prefabId);
                                networkManager.LogError(
                                    $"NetworkBehaviour on index {componentIndex} could nto be found within NetworkObject {nob.name} with ObjectId {objectId}. Prefab {prefab.name} will be instantiated without parent synchronization.");
                            }
                        }
            ...
            else
            {
                ...
                if (result == null)
                    networkManager.LogError(
                        $"ObjectId {cnob.ObjectId} could not be found in Server spawned, nor Server pending despawn.");
            }
        ...
        internal NetworkObject GetSpawnedNetworkObject(CachedNetworkObject cnob)
        {
            ...
            if (Spawned.TryGetValueIL2CPP(cnob.ObjectId, out nob)) return nob;
        ...
        /// </summary>
        private void ReadSpawnedObject(PooledReader reader, out int? parentObjectId, out byte? parentComponentIndex,
            out int? prefabId)
        {
            //Parent.
