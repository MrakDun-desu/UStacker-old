...
namespace FishNet.Managing
{
    public partial class NetworkManager : MonoBehaviour
    {
        ...
        /// </summary>
        /// <typeparam name="T">
        ///     Type of PrefabObjects to return. This is also used to create an instance of type when
        ...
        /// <summary>
        /// </summary>
        private readonly Dictionary<ushort, PrefabObjects> _runtimeSpawnablePrefabs = new();
        ...
        /// </summary>
        private readonly Dictionary<string, List<Action<UnityComponent>>> _pendingInvokes = new();

        /// <summary>
        ///     Currently registered components.
        /// </summary>
        private readonly Dictionary<string, UnityComponent> _registeredComponents = new();
        ...
        /// <summary>
        ///     Invokes an action when a specified component becomes registered. Action will invoke immediately if already
        ///     registered.
        /// </summary>
        ...
        {
            var result = GetInstance<T>(false);
            //If not found yet make a pending invoke.
            if (result == default(T))
            {
                var tName = GetInstanceName<T>();
        ...
        {
            var tName = GetInstanceName<T>();
            if (_registeredComponents.TryGetValue(tName, out var result))
                return (T) result;
            if (warn)
        ...
        public void RegisterInstance<T>(T component, bool replace = true) where T : UnityComponent
        {
            ...
            else
            {
                ...
                if (_pendingInvokes.TryGetValue(tName, out var dels))
                {
        ...
        {
            foreach (var handlers in _pendingInvokes.Values)
