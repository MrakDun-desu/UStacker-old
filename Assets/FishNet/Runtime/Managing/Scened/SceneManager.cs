...
namespace FishNet.Managing.Scened
{
    ...
    public sealed class SceneManager : MonoBehaviour
    {
        ...
        /// <summary>
        ///     Move objects visible to clientHost that are within an unloading scene.This ensures the objects are despawned on the
        ///     client side rather than when the scene is destroyed.
        /// </summary>
        ...
        private void MoveClientHostObjects(Scene scene, bool asServer)
        {
            ...
            var movingNobs = ListCaches.GetNetworkObjectCache();
        ...
        /// <summary>
        ///     Adds a connection to a scene. This will always be called one connection at a time because connections are only
        ///     added after they invidually validate loading the scene.
        ...
        public void AddConnectionToScene(NetworkConnection conn, Scene scene)
        {
            ...
            if (added)
            {
                ...
                NetworkConnection[] arrayConn = {conn};
        ...
        /// <summary>
        ///     Called after the active scene has been set, immediately after scene loads. This will occur before NetworkBehaviour
        ///     callbacks run for the scene's objects.
        ///     The boolean will indicate if the scene set active was specified by the user.
        /// </summary>
        public event Action<bool> OnActiveSceneSet;

        /// <summary>
        ///     Called when a client loads initial scenes after connecting. Boolean will be true if asServer. This will invoke even
        ///     if the SceneManager is not used when the client completes fully connecting to the server.
        /// </summary>
        public event Action<NetworkConnection, bool> OnClientLoadedStartScenes;

        /// <summary>
        ///     Called when a scene change queue has begun. This will only call if a scene has succesfully begun to load or unload.
        ///     The queue may process any number of scene events. For example: if a scene is told to unload while a load is still
        ///     in progress, then the unload will be placed in the queue.
        /// </summary>
        ...
        /// <summary>
        ///     Called when completion percentage changes while loading a scene. Value is between 0f and 1f, while 1f is 100% done.
        ...
        /// <summary>
        ///     True to move objects visible to clientHost that are within an unloading scene. This ensures the objects are
        ///     despawned on the client side rather than when the scene is destroyed.
        /// </summary>
        [Tooltip(
        ...
        /// </summary>
        private readonly List<object> _queuedOperations = new();

        /// <summary>
        ///     Scenes which must be manually unloaded, even when emptied.
        /// </summary>
        private readonly HashSet<Scene> _manualUnloadScenes = new();

        /// <summary>
        ///     Scene containing moved objects when changing single scene. On client this will contain all objects moved until the
        ///     server destroys them.
        ///     The network only sends spawn messages once per-client, per server side scene load. If a scene load is performed
        ///     only for specific connections
        ///     then the server is not resetting their single scene, but rather the single scene for those connections only.
        ///     Because of this, any objects
        ///     which are to be moved will not receive a second respawn message, as they are never destroyed on server, only on
        ///     client.
        ...
        /// <summary>
        ///     Becomes true when when a scene first successfully begins to load or unload. Value is reset to false when the scene
        ///     queue is emptied.
        /// </summary>
        ...
        /// </summary>
        private readonly List<GameObject> _movingObjects = new();
        ...
        /// </summary>
        private readonly Dictionary<NetworkConnection, int> _pendingClientSceneChanges = new();
        ...
        /// <summary>
        ///     Called when authenitcator has concluded a result for a connection. Boolean is true if authentication passed, false
        ///     if failed. This invokes before OnClientAuthenticated so FishNet may run operations on authenticated clients before
        ///     user code does.
        /// </summary>
        ...
        internal void OnClientAuthenticated(NetworkConnection connection)
        {
            ...
            else
            {
                ...
                var qd = new LoadQueueData(SceneScopeType.Global, Array.Empty<NetworkConnection>(), sld, _globalScenes,
                    false);
        ...
        /// <summary>
        ///     Invokes when completion percentage changes while unloading or unloading a scene. Value is between 0f and 1f, while
        ///     1f is 100% done.
        /// </summary>
        /// <param name="value"></param>
        ...
        /// <summary>
        ///     Loads a connection scene queue data. This behaves just like a networked scene load except it sends only to the
        ///     specified connections, and it always loads as an additive scene on server.
        /// </summary>
        /// <returns></returns>
        private IEnumerator __LoadScenes()
        {
            ...
            * unload GlobalScenes if loading as connection. */
            ...
            if (replaceScenes != ReplaceOption.None && !asHost)
                ...
                for (var i = 0; i < UnitySceneManager.sceneCount; i++)
                {
                    ...
                    * into an existing scene. */
                    ...
                    var inScenesCurrent = SceneConnections.TryGetValueIL2CPP(s, out conns);
            ...
            {
                bool byUser;
                var preferredActiveScene = GetUserPreferredActiveScene(sceneLoadData.PreferredActiveScene, out byUser);
                ...
                if (!preferredActiveScene.IsValid())
                    ...
                    if (sceneLoadData.ReplaceScenes != ReplaceOption.None &&
                        data.ScopeType == SceneScopeType.Connections && !_networkManager.IsServer)
                ...
                else if (data.ScopeType == SceneScopeType.Connections)
                {
                    ...
                    for (var i = 0; i < data.Connections.Length; i++)
                        if (data.Connections[i].Authenticated)
                            data.Connections[i].Broadcast(msg);
                }
            }
        ...
        /// <summary>
        ///     Unloads scenes on server and tells a connection to unload them as well. Other connections will not unload this
        ///     scene.
        /// </summary>
        ...
        /// <summary>
        ///     Unloads scenes on server and tells connections to unload them as well. Other connections will not unload this
        ///     scene.
        /// </summary>
        ...
        private IEnumerator __UnloadScenes()
        {
            ...
            * something to do with the coroutine. */
            ...
            bool byUser;
            var preferredActiveScene = GetUserPreferredActiveScene(sceneUnloadData.PreferredActiveScene, out byUser);
            SetActiveScene(preferredActiveScene, byUser);
