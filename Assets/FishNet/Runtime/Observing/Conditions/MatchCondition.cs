...
namespace FishNet.Component.Observing
{
    ...
    public class MatchCondition : ObserverCondition
    {
        ...
        /// </summary>
        private static void FinalizeChange(int match, HashSet<NetworkConnection> remainingConnsInMatch,
            ...
            * and connection share a match. */
            if (owner.IsValid)
            {
                ...
                if (!_connectionMatch.TryGetValueIL2CPP(connection, out var match))
                    ...
                    return !_connectionMatch.TryGetValueIL2CPP(owner, out var _);
                //Match isn't found.
        ...
        /// <summary>
        /// </summary>
        private static readonly Dictionary<int, HashSet<NetworkConnection>> _matchConnections = new();
        ...
        private static readonly Dictionary<NetworkConnection, int> _connectionMatch = new();
        ...
        /// <summary>
        /// </summary>
        private static readonly Dictionary<int, HashSet<NetworkObject>> _matchObjects = new();
        ...
        /// </summary>
        /// //todo this needs to hold hashset so conns can be in multiple matches.
        private static readonly Dictionary<NetworkObject, int> _objectMatch = new();
        ...
        {
            var removed = RemoveFromMatchWithoutRebuild(conn, manager);
            if (removed)
                GetServerObjects(manager).RebuildObservers();
        }
        ...
        {
            var removed = RemoveFromMatchWithoutRebuild(nob, manager);
            if (removed)
