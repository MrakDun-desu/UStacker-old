    ...
    public sealed class NetworkObserver : MonoBehaviour
    {
        ...
        internal void PreInitialize(NetworkObject networkObject)
        {
            if (!_preintiialized)
            {
                ...
                var ignoringManager = OverrideType == ConditionOverrideType.IgnoreManager;
                ...
                * have visibility. */
                if (notOwner)
                {
                    ...
                    else
                    {
                        ...
                        var startNonTimedMet = _nonTimedMet.Contains(connection);
                        ...
                        else
                        {
                            ...
                            for (var i = 0; i < collection.Count; i++)
                            {
                                ...
                                var conditionMet = !condition.GetIsEnabled() ||
                                                   condition.ConditionMet(connection, currentlyAdded, out notProcessed);
        ...
        /// </summary>
        private readonly List<ObserverCondition> _timedConditions = new();

        /// <summary>
        ///     Connections which have all non-timed conditions met.
        /// </summary>
        private readonly HashSet<NetworkConnection> _nonTimedMet = new();
