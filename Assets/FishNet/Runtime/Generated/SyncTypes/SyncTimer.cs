...
namespace FishNet.Object.Synchronizing
{
    ...
    public class SyncTimer : SyncBase, ICustomSync
    {
        ...
        public void PauseTimer(bool sendRemaining = false)
        {
            ...
            var op = sendRemaining ? SyncTimerOperation.PauseUpdated : SyncTimerOperation.Pause;
        ...
        public void StopTimer(bool sendRemaining = false)
        {
            ...
            var op = sendRemaining ? SyncTimerOperation.StopUpdated : SyncTimerOperation.Stop;
        ...
        private void AddOperation(SyncTimerOperation operation, float prev, float next)
        {
            ...
            var asServerInvoke = !IsNetworkInitialized || NetworkBehaviour.IsServer;

            if (asServerInvoke)
                if (Dirty())
                {
                    var change = new ChangeData(operation, prev, next);
                    _changed.Add(change);
                }
        ...
        public override void WriteDelta(PooledWriter writer, bool resetSyncTick = true)
        {
            ...
            for (var i = 0; i < _changed.Count; i++)
            {
                ...
                else if (change.Operation == SyncTimerOperation.PauseUpdated ||
                         change.Operation == SyncTimerOperation.StopUpdated) writer.WriteSingle(change.Next);
            }
        ...
        {
            return asServer || !NetworkManager.IsServer;
        }
        ...
        public override void Read(PooledReader reader, bool asServer)
        {
            ...
            {
                var op = (SyncTimerOperation) reader.ReadByte();
                if (op == SyncTimerOperation.Start)
                {
                    var next = reader.ReadSingle();
            ...
            {
                var newPauseState = op == SyncTimerOperation.Pause || op == SyncTimerOperation.PauseUpdated;
        ...
        /// </summary>
        private readonly List<ChangeData> _serverOnChanges = new();

        /// <summary>
        ///     Client OnChange events waiting for start callbacks.
        /// </summary>
        private readonly List<ChangeData> _clientOnChanges = new();
