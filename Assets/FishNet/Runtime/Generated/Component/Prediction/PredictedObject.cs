...
namespace FishNet.Component.Prediction
{
    ...
    public partial class PredictedObject : NetworkBehaviour
    {
        ...
        /// </summary>
        public bool IsRigidbodyPrediction => _predictionType == PredictionType.Rigidbody ||
                                             _predictionType == PredictionType.Rigidbody2D;
        ...
        {
            transform.SetTransformOffsets(_graphicalObject, ref _graphicalInstantiatedOffsetPosition,
        ...
        private void InitializeSmoother(bool ownerSmoother)
        {
            ...
            if (ownerSmoother)
            {
                ...
                var teleportThreshold = _enableTeleport ? _teleportThreshold : -1f;
                _ownerSmoother.Initialize(this, _graphicalInstantiatedOffsetPosition,
                    _graphicalInstantiatedOffsetRotation, _graphicalObject, _ownerSmoothPosition, _ownerSmoothRotation,
                    _ownerInterpolation, teleportThreshold);
            }
            else
            {
                _spectatorSmoother = new PredictedObjectSpectatorSmoother();
                var rbType = _predictionType == PredictionType.Rigidbody
                    ? RigidbodyType.Rigidbody
                    : RigidbodyType.Rigidbody2D;
                var teleportThreshold = _enableTeleport ? _teleportThreshold : -1f;
                _spectatorSmoother.Initialize(this, rbType, _rigidbody, _rigidbody2d, _graphicalObject,
                    _spectatorSmoothPosition, _spectatorSmoothRotation, _spectatorSmoothingDuration,
                    _spectatorInterpolation, _overflowMultiplier, teleportThreshold);
            }
        ...
        /// </summary>
        [Tooltip("True if this object implements replicate and reconcile methods.")] [SerializeField]
        ...
        /// </summary>
        [Tooltip("True to smooth position on owner objects.")] [SerializeField]
        private bool _ownerSmoothPosition = true;

        /// <summary>
        ///     True to smooth rotation on owner objects.
        /// </summary>
        [Tooltip("True to smooth rotation on owner objects.")] [SerializeField]
        ...
        /// </summary>
        /// <param name="asOwner">
        ///     True to get the interpolation for when owner, false to get the interpolation for when a
        ///     spectator.
        /// </param>
        public byte GetInterpolation(bool asOwner)
        {
            return asOwner ? _ownerInterpolation : _spectatorInterpolation;
        }
        ...
        /// </summary>
        [Tooltip("True to smooth position on spectated objects.")] [SerializeField]
        private bool _spectatorSmoothPosition = true;

        /// <summary>
        ///     True to smooth rotation on spectated objects.
        /// </summary>
        [Tooltip("True to smooth rotation on spectated objects.")] [SerializeField]
        ...
        /// </summary>
        [Tooltip(
            "Multiplier applied to difference in velocity between ticks. Positive values will result in more velocity while lowers will result in less. A value of 1f will prevent any velocity from being lost between ticks, unless indicated by the server.")]
        [Range(-10f, 10f)]
        [SerializeField]
        private float _maintainedVelocity;

        /// <summary>
        ///     How often to resend current values regardless if the state has changed. Using this value will consume more
        ///     bandwidth but may be preferred if you want to force synchronization the object move on the client but not on the
        ///     server.
        /// </summary>
        [Tooltip(
        ...
        /// </summary>
        [Tooltip("How often in ticks to resend values.")] [SerializeField]
        private ushort _resendInterval = 30;

        /// <summary>
        ///     NetworkTransform to configure.
        /// </summary>
        [Tooltip("NetworkTransform to configure.")] [SerializeField]
