...
namespace FishNet.Component.Prediction
{
    internal class PredictedObjectSpectatorSmoother
    {
        ...
        /// </summary>
        internal void Initialize(PredictedObject po, RigidbodyType rbType, Rigidbody rb, Rigidbody2D rb2d,
            Transform graphicalObject
            , bool smoothPosition, bool smoothRotation, float smoothingDuration, byte interpolation,
        ...
        public void OnPreTick()
        {
            if (CanSmooth())
            {
                if (!_preTickReceived)
                {
                    var tick = _predictedObject.TimeManager.LocalTick - 1;
        ...
        {
            var positionMatches = !_smoothPosition || _graphicalObject.position == localPosition;
            var rotationMatches = !_smoothRotation || _graphicalObject.rotation == localRotation;
        ...
        private bool HasChanged(TransformData td)
        {
            ...
            var changed = td.Position != rigidbodyTransform.position || td.Rotation != rigidbodyTransform.rotation;
        ...
        private void MoveToTarget(float deltaOverride = -1f)
        {
            ...
            var queueCount = _goalDatas.Count;
            ...
            var multiplier = 1f;
            var countOverInterpolation = queueCount - _interpolation;
            ...
            {
                var speedupCompletePercent = Mathf.InverseLerp(_smoothingDuration, 0f, _speedUpTimeRemaining);
                multiplier *= speedupCompletePercent;
                var nextValue = _speedUpTimeRemaining - delta;
            ...
            {
                var subtractionAmount = delta * multiplier;
                var timeRemaining = rd.TimeRemaining - subtractionAmount;
        ...
        public void CreateGoalData(uint tick, bool postTick)
        {
            ...
            var maximumBufferAllowance = _interpolation * 8;
            var removedBufferCount = _goalDatas.Count - maximumBufferAllowance;
            //If there are some to remove.
            if (removedBufferCount > 0)
            {
                for (var i = 0; i < removedBufferCount; i++)
                    StoreGoalData(_goalDatas[0 + i]);
                _goalDatas.RemoveRange(0, removedBufferCount);
            }

            var currentGoalDataTick = _currentGoalData.LocalTick;
            ...
            var datasCount = _goalDatas.Count;
            ...
            var injectionIndex = datasCount + 1;
            ...
            var bufferFillPercent = _goalDatas.Count / (float) _interpolation;
            var speedUpRemaining = Mathf.Lerp(_smoothingDuration, 0f, bufferFillPercent);
            ...
            var nextGoalData = RetrieveGoalData();
            nextGoalData.LocalTick = tick;
            //Set next transform data.
            var nextTd = nextGoalData.Transforms;
            if (_rigidbodyType == RigidbodyType.Rigidbody)
        ...
        private GoalData GetGoalData(uint tick, out int index)
        {
            ...
            for (var i = 0; i < _goalDatas.Count; i++)
        ...
        /// </summary>
        private readonly GoalData _currentGoalData = new();
        ...
        /// </summary>
        private readonly List<GoalData> _goalDatas = new();
        ...
        /// </summary>
        private readonly TransformData _preTickTransformdata = new();
        ...
        /// </summary>
        private static readonly Stack<GoalData> _goalDataCache = new();
        ...
        private void SetCalculatedRates(GoalData prevGoalData, GoalData nextGoalData, Channel channel)
        {
            ...
            var lastTick = prevGoalData.LocalTick;
            ...
            var tickDifference = nextGoalData.LocalTick - lastTick;
            var timePassed = (float) _predictedObject.TimeManager.TicksToTime(tickDifference);
            var nextRd = nextGoalData.Rates;

            //Distance between properties.
            float distance;
            //Position.
            var lastPosition = prevGoalData.Transforms.Position;
            ...
            var positionRate = distance / timePassed;
            //Rotation.
            distance = prevGoalData.Transforms.Rotation.Angle(nextTd.Rotation, true);
            var rotationRate = distance / timePassed;
