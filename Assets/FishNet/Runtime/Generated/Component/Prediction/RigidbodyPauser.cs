    ...
    public class RigidbodyPauser
    {
        ...
        public void UpdateRigidbodies(Transform t, RigidbodyType rbType, bool getInChildren, Transform graphicalObject)
        {
            ...
            if (rbType == RigidbodyType.Rigidbody)
            {
                ...
                for (var i = 0; i < _rigidbodyDatas.Count; i++)
                    if (_rigidbodyDatas[i].Rigidbody.transform == graphicalObject)
                    {
                        NetworkManager.StaticLogError(
                            $"GameObject {t.name} has it's GraphicalObject as a child or on the same object as a Rigidbody object. The GraphicalObject must be a child of root, and not sit beneath or on any rigidbodies.");
            ...
            else
            {
                if (getInChildren)
                {
                    var rbs = t.GetComponentsInChildren<Rigidbody2D>();
                    for (var i = 0; i < rbs.Length; i++)
                ...
                //Make sure all added datas are not the graphical object.
                for (var i = 0; i < _rigidbody2dDatas.Count; i++)
        ...
        public void Unpause()
        {
            ...
            if (_rigidbodyType == RigidbodyType.Rigidbody)
            {
                ...
                {
                    var rbData = _rigidbodyDatas[index];
            ...
            else
            {
                ...
                bool UnpauseRigidbody(int index)
                {
                    var rbData = _rigidbody2dDatas[index];
                    var rb = rbData.Rigidbody2d;
                    if (rb == null)
                        return false;
        ...
        public void Pause()
        {
            ...
            var kinematicScene = _kinematicScene;
        ...
        /// </summary>
        private readonly List<RigidbodyData> _rigidbodyDatas = new();

        /// <summary>
        ///     Rigidbody2D datas for found rigidbodies;
        /// </summary>
        private readonly List<Rigidbody2DData> _rigidbody2dDatas = new();
        ...
        private static Scene _kinematicScene
        {
            get
            {
                if (!_kinematicSceneCache.IsValid())
                    _kinematicSceneCache = SceneManager.CreateScene("RigidbodyPauser_Kinematic",
