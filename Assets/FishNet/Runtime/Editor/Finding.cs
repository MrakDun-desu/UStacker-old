...
namespace FishNet.Editing
{
    public static class Finding
    {
        ...
        private static void SetPaths(bool error)
        {
            ...
            foreach (var item in objectPaths)
            {
                ...
                if (item.ToLower().Contains(runtimeName))
                    _fishNetRuntimePath = Path.GetDirectoryName(item);
                else if (item.ToLower().Contains(generatedName))
                    _fishNetGeneratedPath = Path.GetDirectoryName(item);
        ...
        /// </summary>
        [NonSerialized] private static string _fishNetRuntimePath = string.Empty;
