...
namespace FishNet.Editing.PrefabCollectionGenerator
{
    internal sealed class Generator : AssetPostprocessor
    {
            ...
            string[] movedAssets, string[] movedFromAssetPaths)
        {
            ...
            var prefabCollection = GetDefaultPrefabObjects();
            if (prefabCollection == null)
                return;
            var settings = Configuration.Configurations.PrefabGenerator;
            ...
            {
                var totalChanges = importedAssets.Length + deletedAssets.Length + movedAssets.Length +
                ...
                //normalizes path.
                var dpoPath = Path.GetFullPath(settings.DefaultPrefabObjectsPath);
                ...
                if (totalChanges == 1)
                {
                    ...
                    var imported = importedAssets.Length == 1 ? importedAssets[0] : null;
                    if (imported != null && imported == _lastSingleImportedAsset)
                    {
                        ...
                        var assetType = AssetDatabase.GetMainAssetTypeAtPath(imported);
                        //Not a gameObject, no reason to continue.
                        if (assetType != typeof(GameObject))
                            return;

                        var nob = AssetDatabase.LoadAssetAtPath<NetworkObject>(imported);
                ...
                var fullRebuild = settings.FullRebuild;
                ...
                * safe than sorry. */
                ...
                {
                    var targetText = "VERSION.txt".ToLower();
                    ...
                    for (var i = 0; i < arr.Length; i++)
                    {
                        ...
                        item = item.ToLower();
                        var startIndex = itemLength - targetLength;
        ...
        public static string[] GetPrefabFiles(string startingPath, HashSet<string> excludedPaths, bool recursive)
        {
            ...
            {
                var strResults = Directory.GetFiles(startingPath, "*.prefab", SearchOption.AllDirectories);
                return strResults;
            }
            ...
            var enumeratedCollection = new List<string> {startingPath};
            //Only check other directories if recursive.
            if (recursive)
                //Find all folders which aren't excluded.
                for (var i = 0; i < enumeratedCollection.Count; i++)
                {
                    var allFolders =
                        Directory.GetDirectories(enumeratedCollection[i], "*", SearchOption.TopDirectoryOnly);
                    for (var z = 0; z < allFolders.Length; z++)
                    {
                        var current = allFolders[z];
                        //Not excluded.
            ...
            var count = enumeratedCollection.Count;
            for (var i = 0; i < count; i++)
            {
                var r = Directory.GetFiles(enumeratedCollection[i], "*.prefab", SearchOption.TopDirectoryOnly);
                results.AddRange(r);
            }
        ...
        private static void RemoveOverlappingFolders(List<SpecifiedFolder> folders)
        {
            ...
            for (var i = 0; i < folders.Count; i++)
            {
                ...
                //Duplicate.
                if (folders[z].Path.Equals(folders[i].Path, StringComparison.OrdinalIgnoreCase))
                {
                    UnityDebug.LogError(
                        "The same path is specified multiple times in the DefaultPrefabGenerator settings. Remove the duplicate to clear this error.");
                    folders.RemoveAt(i);
                    break;
                }
                ...
                //Compare paths.
                var zPath = GetPathWithSeparator(folders[z].Path);
                var iPath = zPath.Substring(0, zPath.Length);
                //If paths match.
                if (iPath.Equals(zPath, StringComparison.OrdinalIgnoreCase))
                {
                    UnityDebug.LogError(
                        $"Path {folders[i].Path} is included within recursive path {folders[z].Path}. Remove path {folders[i].Path} to clear this error.");
                    folders.RemoveAt(i);
                    break;
                }
            }
        ...
        /// </summary>
        public static void GenerateChanged(string[] importedAssets, string[] deletedAssets, string[] movedAssets,
            string[] movedFromAssetPaths, PrefabGeneratorConfigurations settings = null)
        {
            if (settings == null)
            ...
            var log = settings.LogToConsole;
            var sw = log ? Stopwatch.StartNew() : null;

            var prefabCollection = GetDefaultPrefabObjects(settings);
            //No need to error if nto found, GetDefaultPrefabObjects will.
            if (prefabCollection == null)
                return;

            var assetsLength = importedAssets.Length + deletedAssets.Length + movedAssets.Length +
                               movedFromAssetPaths.Length;
            var changedNobPaths = new List<string>();
            ...
            var startCount = prefabCollection.GetObjectCount();
            ...
            var firstAddIndex = prefabCollection.GetObjectCount() - 1;
            ...
            void IterateAssetCollection(string[] c)
            {
                foreach (var item in c)
                {
                    ...
                    var nob = AssetDatabase.LoadAssetAtPath<NetworkObject>(item);
                    if (nob != null)
                    {
            ...
            if (log && dirtied)
                UnityDebug.Log(
                    $"Default prefab generator updated prefabs in {sw.ElapsedMilliseconds}ms.{GetDirtiedMessage(settings, dirtied)}");

            //Check for redundancy.
            var frameCount = Time.frameCount;
            var changedCount = changedNobPaths.Count;
            if (frameCount == _lastUpdatedFrame && assetsLength == _lastUpdatedLengths &&
                changedCount == _lastUpdatedNamePaths.Count && changedCount > 0)
            {
                var allMatch = true;
        ...
        public static void GenerateFull(PrefabGeneratorConfigurations settings = null, bool forced = false)
        {
            ...
            var excludedPaths = new HashSet<string>(settings.ExcludedFolders);

            //If searching the entire project.
            if (settings.SearchScope == (int) SearchScopeType.EntireProject)
            {
                foreach (var path in GetPrefabFiles("Assets", excludedPaths, true))
                {
            ...
            {
                var folders = GetSpecifiedFolders(settings.IncludedFolders.ToList());
                ...
                foreach (var sf in folders)
                {
                    ...
                    foreach (var path in GetPrefabFiles(sf.Path, excludedPaths, sf.Recursive))
                    {
                        var nob = AssetDatabase.LoadAssetAtPath<NetworkObject>(path);
                        if (nob != null)
                            foundNobs.Add(nob);
            ...
            {
                UnityDebug.LogError(
                    $"{settings.SearchScope} is not handled; default prefabs will not generator properly.");
            }
            ...
            prefabCollection.AddObjects(foundNobs);
            var dirtied = prefabCollection.SetAssetPathHashes(0);

            var newCount = prefabCollection.GetObjectCount();
            if (log)
            {
                var dirtiedMessage = newCount > 0 ? GetDirtiedMessage(settings, dirtied) : string.Empty;
                UnityDebug.Log(
                    $"Default prefab generator found {newCount} prefabs in {sw.ElapsedMilliseconds}ms.{dirtiedMessage}");
            }
        ...
        {
            var results = new List<SpecifiedFolder>();
            //Remove astericks.
        ...
        private static DefaultPrefabObjects GetDefaultPrefabObjects(PrefabGeneratorConfigurations settings = null)
        {
            ...
            var defaultPrefabsPath = settings.DefaultPrefabObjectsPath;
            var fullDefaultPrefabsPath =
                defaultPrefabsPath.Length > 0 ? Path.GetFullPath(defaultPrefabsPath) : string.Empty;
            //If cached prefabs is not the same path as assetPath.
            if (_cachedDefaultPrefabs != null)
            {
                var unityAssetPath = AssetDatabase.GetAssetPath(_cachedDefaultPrefabs);
                var fullCachedPath = unityAssetPath.Length > 0 ? Path.GetFullPath(unityAssetPath) : string.Empty;
            ...
            if (_cachedDefaultPrefabs == null)
                ...
                if (File.Exists(fullDefaultPrefabsPath))
                {
                    ...
                    if (_cachedDefaultPrefabs == null)
                    {
                        ...
                        {
                            UnityDebug.LogError(
                                "DefaultPrefabObjects file exists but it could not be loaded by Unity. Use the Fish-Networking menu to Refresh Default Prefabs.");
                        }
                        else
                        {
                            UnityDebug.Log(
                                "DefaultPrefabObjects file exists but it could not be loaded by Unity. Trying to reload the file next frame.");
        ...
        /// </summary>
        [NonSerialized] private static bool _ranOnce;

        /// <summary>
        ///     Last paths of updated nobs during a changed update.
        /// </summary>
        [NonSerialized] private static List<string> _lastUpdatedNamePaths = new();

        /// <summary>
        ///     Last frame changed was updated.
        /// </summary>
        [NonSerialized] private static int _lastUpdatedFrame = -1;

        /// <summary>
        ///     Length of assets strings during the last update.
        /// </summary>
        [NonSerialized] private static int _lastUpdatedLengths = -1;
