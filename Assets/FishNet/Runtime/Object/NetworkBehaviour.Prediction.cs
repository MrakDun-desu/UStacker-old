...
namespace FishNet.Object
{
    public abstract partial class NetworkBehaviour : MonoBehaviour
    {
        ...
        private void SendReplicateRpc<T>(uint hash, List<T> replicates, Channel channel) where T : IReplicateData
        {
            ...
            var bufferCount = replicates.Count;
            var lastBufferIndex = bufferCount - 1;
            //Nothing to send; should never be possible.
            if (lastBufferIndex < 0)
                return;

            //Number of past inputs to send.
            var pastInputs = Mathf.Min(PredictionManager.GetRedundancyCount(), bufferCount);
            /* Where to start writing from. When passed
             * into the writer values from this offset
             * and forward will be written. */
            var offset = bufferCount - pastInputs;
            if (offset < 0)
                offset = 0;

            var lastReplicateTick = _lastSentReplicateTick;
            if (lastReplicateTick > 0)
            {
                var diff = TimeManager.LocalTick - GetLastReplicateTick();
                offset += (int) diff - 1;
            ...
            var methodWriter = WriterPool.GetWriter(WriterPool.LENGTH_BRACKET);
        ...
        [CodegenMakePublic] //internal
        [APIExclude]
        public void Replicate_Server_Internal<T>(ReplicateUserLogicDelegate<T> del, Queue<T> q, Channel channel)
            where T : IReplicateData
        {
            ...
            if (count > 0)
            {
                ...
                var consumeExcess = !pm.DropExcessiveReplicates;
                ...
                {
                    var maximumAllowedConsumes = pm.GetMaximumConsumeCount();
        ...
        /// </summary>
        /// <returns></returns>
        private int GetMaximumReplicatesWhenConsuming()
        {
            return TimeManager.TickRate * 3;
        }
        ...
        [CodegenMakePublic] //internal
        [APIExclude]
        public void Replicate_Client_Internal<T>(ReplicateUserLogicDelegate<T> del, uint methodHash, List<T> replicates,
            T data, Channel channel) where T : IReplicateData
        {
            ...
            if (!IsServer)
            {
                ...
                var mayChange = TransformMayChange();
                var resetResends = pm.UsingRigidbodies || mayChange || !isDefault;
                ...
                var enqueueData = _remainingResends > 0;
                if (enqueueData)
                {
                    ...
                    var maximumReplicates = TimeManager.TickRate * 2;
                    //If over then remove half the replicates.
                    if (replicates.Count >= maximumReplicates)
                    {
                        var removeCount = maximumReplicates / 2;
                        //Dispose first.
                ...
                {
                    _remainingResends--;
                    SendReplicateRpc(methodHash, replicates, channel);
        ...
        /// </summary>
        public void Replicate_Reader_Internal<T>(PooledReader reader, NetworkConnection sender, T[] arrBuffer,
            Queue<T> replicates, Channel channel) where T : IReplicateData
        {
            var receivedReplicatesCount = reader.ReadReplicate(ref arrBuffer, TimeManager.LastPacketTick);
            ...
            var replicatesCountLimit =
                consumeExcess ? GetMaximumReplicatesWhenConsuming() : pm.GetMaximumServerReplicates();
        ...
        /// </summary>
        public void Reconcile_Client_Internal<T, T2>(ReconcileUserLogicDelegate<T> reconcileDel,
            ReplicateUserLogicDelegate<T2> replicateULDel, List<T2> replicates, T data, Channel channel)
            where T : IReconcileData where T2 : IReplicateData
        {
            var tick = data.GetTick();
            ...
            var ps2d = scene.GetPhysicsScene2D();
            ...
            var tmPhysics = TimeManager.PhysicsMode == PhysicsMode.TimeManager;
            ...
            var queueIndex = -1;
            for (var i = 0; i < replicates.Count; i++)
            ...
            var replays = replicates.Count;
            var tickDelta = (float) TimeManager.TickDelta;

            for (var i = 0; i < replays; i++)
            {
                var rData = replicates[i];
                var replayTick = rData.GetTick();
        ...
        /// </summary>
        public void Reconcile_Reader_Internal<T>(PooledReader reader, ref T data, Channel channel)
        ...
        /// <summary>
        ///     Number of resends which may occur. This could be for client resending replicates to the server or the server
        ///     resending reconciles to the client.
        /// </summary>
