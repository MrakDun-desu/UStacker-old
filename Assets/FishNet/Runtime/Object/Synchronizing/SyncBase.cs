...
namespace FishNet.Object.Synchronizing.Internal
{
    public class SyncBase : ISyncType
    {
        ...
        protected bool CanNetworkSetValues(bool warn = true)
        {
            ...
            if (NetworkManager != null && NetworkManager.PredictionManager.GetAllowPredictedSpawning() &&
                NetworkBehaviour.NetworkObject.AllowPredictedSpawning)
                return true;
            /* If here then server is not active and additional
             * checks must be performed. */
            var result = Settings.ReadPermission == ReadPermission.ExcludeOwner && NetworkBehaviour.IsOwner;
            if (!result && warn)
        ...
        /// <summary>
        ///     Channel to use for next write. To ensure eventual consistency this eventually changes to reliable when Settings are
        ///     unreliable.
        /// </summary>
