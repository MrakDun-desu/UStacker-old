    ...
    internal class WriterProcessor : CodegenBase
    {
        ...
        public bool IsSpecialWriteMethod(SR.MethodInfo methodInfo)
        {
            ...
            if (methodInfo.Name == nameof(PooledWriter.WriteDictionary))
                return true;
            if (methodInfo.Name == nameof(PooledWriter.WriteList))
                return true;
        ...
        public void AddWriterMethod(TypeReference typeRef, MethodReference methodRef, bool instanced, bool useAdd)
        {
            ...
            var fullName = typeRef.GetFullnameWithoutBrackets();
            if (useAdd)
...
#pragma warning restore CS0219
        ...
        {
            var writerPd = staticWriterMd.Parameters[0];
            var valuePd = staticWriterMd.Parameters[1];
            var processor = staticWriterMd.Body.GetILProcessor();
            processor.Emit(OpCodes.Ldarg, writerPd);
            processor.Emit(OpCodes.Ldarg, valuePd);
        ...
        {
            var wi = GetClass<WriterImports>();
            var valueTr = objectTr.GetElementType();

            //Write not found.
            if (GetOrCreateWriteMethodReference(valueTr) == null)
                return null;

            var createdMd = CreateStaticWriterStubMethodDefinition(objectTr);
            AddToStaticWriters(objectTr, createdMd);

            //Find instanced writer to use.
            var instancedWriteMr = wi.Writer_WriteArray_MethodRef;
            //Make generic.
            var writeGim = instancedWriteMr.MakeGenericMethod(valueTr);
        ...
        private MethodReference CreateGenericCollectionWriterMethodReference(TypeReference objectTr, SerializerType st)
        {
            ...
            var valueTr = genericInstance.GenericArguments[0];

            var genericArguments = new List<TypeReference>();
            //Make sure all arguments have writers.
            foreach (var gaTr in genericInstance.GenericArguments)
            {
                var mr = GetOrCreateWriteMethodReference(gaTr);
                //Writer not found.
                if (mr == null)
                {
            ...
            var valueWriteMr = GetOrCreateWriteMethodReference(valueTr);
            ...
            //Make generic.
            var writeGim = instancedWriteMr.MakeGenericMethod(genericArguments.ToArray());
            CallInstancedWriter(createdMd, writeGim);
