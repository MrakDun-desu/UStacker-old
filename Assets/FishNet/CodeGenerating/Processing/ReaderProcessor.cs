...
namespace FishNet.CodeGenerating.Helping
{
    internal class ReaderProcessor : CodegenBase
    {
            ...
            TypeReference readTypeRef, out VariableDefinition createdVariableDef)
        {
            ...
            var readMr = GetOrCreateReadMethodReference(readTypeRef);
            if (readMr != null)
            {
        ...
        private MethodReference CreateArrayReaderMethodReference(TypeReference objectTr)
        {
            ...
            var createdMd = CreateStaticReaderStubMethodDefinition(objectTr);
            AddToStaticReaders(objectTr, createdMd);

            //Find instanced writer to use.
            var instancedWriteMr = ri.Reader_ReadArray_MethodRef;
            //Make generic.
        ...
        private MethodReference CreateGenericCollectionReaderMethodReference(TypeReference objectTr, SerializerType st)
        {
            ...
            var valueWriteMr = GetOrCreateReadMethodReference(valueTr);
            ...
            //Make generic.
            var writeGim = instancedReadMr.MakeGenericMethod(genericArguments.ToArray());
            CallInstancedReader(createdMd, writeGim);
        ...
        {
            var readerPd = staticReaderMd.Parameters[0];
            var processor = staticReaderMd.Body.GetILProcessor();
            processor.Emit(OpCodes.Ldarg, readerPd);
